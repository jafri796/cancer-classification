"""
Clinical validation on PCam center-region detection.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Clinical validation for PCam model")
    ...
    parser.add_argument("--ensemble", action="store_true", help="Use ensemble from deployment config")
    ...
    results = validator.validate(all_labels, all_probs)

    logger.info(f"Clinical validation for {model_name} ({args.split})")
    logger.info(results)
